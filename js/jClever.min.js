...
c+';"><ul class="jClever-element-select-list"></ul></div>');var g=a(b).parents(".jClever-element").attr("tabindex",d),i=g.find(".jClever-element-select-center");g.find(".jClever-element-select-right");var k=g.find(".jClever-element-select-list"),j=g.find(".jClever-element-select-list-wrapper"),c=a("label[for="+a(b).attr("id")+"]");g.append(e.errorTemplate);a(b).find("option").each(function(){a(this).is(":selected")?g.find(".jClever-element-select-list").append(a('<li class="active" data-value="'+
a(this).val()+'"><span><i>'+a(this).text()+"</i></span></li>")):g.find(".jClever-element-select-list").append(a('<li data-value="'+a(this).val()+'"><span><i>'+a(this).text()+"</i></span></li>"))});a(b).find(":selected")?i.text(a(b).find("option:selected").text()):i.text(a(b).find("option:eq(0)").text());g.on("click.jClever",".jClever-element-select-center, .jClever-element-select-right",function(){j.is(":visible")?a(".jClever-element-select-list-wrapper").hide():(a(".jClever-element-select-list-wrapper").hide(),
j.show(),l[a(b).attr("name")]=j.jScrollPane().data("jsp"))});j.on("blur.jClever",function(){a(this).hide()});g.on("click.jClever","li",function(){var c=a(this).attr("data-value");k.find("li.active").removeClass("active");a(this).addClass("active");a(b).find("option").removeAttr("selected");a(b).find('option[value="'+c+'"]').attr("selected","selected");a(b).trigger("change");j.hide();return!1});a(b).on("change.jClever",function(){i.text(a(this).find(":selected").text())});a(b).on("update.jClever",
...
function(){g.trigger("focus");j.show();l[a(b).attr("name")]=j.jScrollPane().data("jsp")});var n="",p=null;g.on("keydown.jClever",function(c){var d=a(b)[0].selectedIndex;switch(c.keyCode){case 40:d<a(b).find("option").length-1&&d++;break;case 38:0<d&&d--;break;case 13:j.is(":visible")?j.hide():(j.show(),l[a(b).attr("name")]=j.jScrollPane().data("jsp"));break;case 32:j.is(":visible")?j.hide():(j.show(),l[a(b).attr("name")]=j.jScrollPane().data("jsp"));break;case 9:return j.hide(),!0;default:n+=String.fromCharCode(c.keyCode),
clearTimeout(p),p=setTimeout(function(){for(var c=0,f=a(b)[0].options.length,e=0;e<f;e++)if("string"==typeof a(b)[0].options[e].text){var h=a(b)[0].options[e].text.toUpperCase();if(RegExp("^"+n,"i").test(h)){d=c;a(b)[0].selectedIndex=d;g.find("li.selected").removeClass("selected");g.find("li:eq("+d+")").addClass("selected");g.find("option").removeAttr("selected");g.find("option:eq("+d+")").attr("selected","selected");a(b).trigger("change");j.is(":visible")&&l[a(b).attr("name")].scrollToElement(g.find("li:eq("+
d+")"));break}c++}n=""},500)}a(b)[0].selectedIndex=d;g.find("li.selected").removeClass("selected");g.find("li:eq("+d+")").addClass("selected");g.find("option").removeAttr("selected");g.find("option:eq("+d+")").attr("selected","selected");a(b).trigger("change");j.is(":visible")&&l[a(b).attr("name")].scrollToElement(g.find("li:eq("+d+")"));return!1})},checkboxActivate:function(b,c){var d=a(b).wrap('<div class="jClever-element" tabindex="'+c+'">').addClass("hidden").after('<span class="jClever-element-checkbox-twins"><span class="jClever-element-checkbox-twins-element"></span><span class="jClever-element-checkbox-twins-color"></span></span>'),
...
a(b).on("focusin.jClever",function(){a(this).parents(".jClever-element").addClass("focused")});a(b).on("focusout.jClever",function(){a(this).parents(".jClever-element").removeClass("focused")})},elementAdd:function(b,c,d){switch(c){case "select":h.selectActivate(b,p,i);b=a.extend(m,d.selectCollection);d.selectCollection=b;break;case "checkbox":h.checkboxActivate(b,i);break;case "radio":h.radioActivate(b,i)}return d}},q={};return this.each(function(){if(!a(this).hasClass("clevered"))return a(this).addClass("clevered").addClass(e.selfClass),
h.init(this),q={selectCollection:m,destroy:function(a){h.destroy(a)},reset:function(a){h.reset(a)},selectSetPosition:function(b,c){a(b).length&&h.selectSetPosition(a(b),c)},selectAdd:function(a){h.selectAdd(a)},checkboxSetState:function(b,c){if(a(b).length)h.checkboxSetState(a(b),c);else return!1},radioSetState:function(b,c){if(a(b).length)h.radioSetState(a(b),c);else return!1},elementAdd:function(a,c,d){return h.elementAdd(a,c,d)}},m={},a.data(a(this).get(0),"publicApi",q),!0})};a.fn.jCleverAPI=
function(e){if(1<this.length)return!1;for(var m=a.data(a(this).get(0),"publicApi"),l=[],k=1;k<arguments.length;k++)l[k-1]=arguments[k];return"function"==typeof m[e]?(1==arguments.length?l=Array(a(this)):l[arguments.length-1]=m,l=m[e].apply(arguments.callee,l),"object"==typeof l?a.data(a(this).get(0),"publicApi",l):l=m,l):m[e]};jQuery.jClever=!0;a(document).mousedown(function(e){0===a(e.target).parents(".jClever-element-select-wrapper").length&&a(".jClever-element-select-list-wrapper:visible").hide()})})(jQuery);