...
var d={init:function(m){if(j.validate.state===true){a(m).submit(function(){var u=a(m).get(0);var w={};a(u).find(".error").removeClass("error");for(var r in j.validate.items){if(!(u[r]===undefined)){for(var p in j.validate.items[r]){switch(p){case"required":if(u[r].value==""||u[r].value==a(u[r]).data("placeholder")){w[r]=j.validate.items[r][p];
}break;case"numeric":if(e.isNumeric(u[r].value)!=true){w[r]=j.validate.items[r][p];}break;case"str":if(e.isString(u[r].value)!=true){w[r]=j.validate.items[r][p];
}break;case"stringAndNumeric":if(e.isAnyText(u[r].value)!=true){w[r]=j.validate.items[r][p];}break;case"mail":if(e.isEmail(u[r].value)!=true){w[r]=j.validate.items[r][p];
}break;case"siteURL":if(e.isSiteURL(u[r].value)!=true){w[r]=j.validate.items[r][p];}break;}}}}var q=0;for(var v in w){if(u[v]!=undefined){var o=w[v];var t=a(u[v]);
var n=t.parents(".jClever-element");var s=n.find("."+j.errorClassTemplate);n.addClass("error");s.text(o);q++;}}if(q){return false;}else{return true;}});
}a(m).find("input[type=text], input[type=password], textarea").each(function(){var o=a(this);var n=a(this).data("placeholder");if(typeof n==="string"){if(o.val()==""){o.val(n).addClass("placeholdered");
}o.focusin(function(){if(o.val()==n){o.val("").removeClass("placeholdered");}});o.focusout(function(){if(o.val()==""){o.val(n).addClass("placeholdered");
}});}});var l=a(m).get(0).elements;for(key=0;key<l.length;key++){switch(l[key].nodeName){case"SELECT":if(j.applyTo.select){var k=a(l[key]);if(typeof k.attr("multiple")=="undefined"){d.selectActivate(l[key],b,g);
}else{d.multiSelectActivate(l[key],b,g);}c[k.attr("name")]={type:"select",value:k.attr("value")};b--;g++;}break;case"TEXTAREA":if(j.applyTo.textarea){var k=a(l[key]);
d.textareaActivate(l[key],g);g++;}break;case"INPUT":var k=a(l[key]);if(j.applyTo.checkbox&&k.attr("type")=="checkbox"){c[k.attr("name")]={type:"checkbox",value:(k.is(":checked")?1:0)};
d.checkboxActivate(l[key],g);g++;}if(j.applyTo.radio&&k.attr("type")=="radio"){c[k.attr("name")]={type:"radio",value:(k.is(":checked")?1:0)};d.radioActivate(l[key],g);
g++;}if(j.applyTo.file&&k.attr("type")=="file"){c[k.attr("name")]={type:"file",value:""};d.fileActivate(l[key],g);g++;}if(j.applyTo.button&&(k.attr("type")=="submit"||k.attr("type")=="reset"||k.attr("type")=="button")){d.submitActivate(l[key],g);
g++;}if(j.applyTo.input&&(k.attr("type")=="text"||k.attr("type")=="password")){d.inputActivate(l[key],g);g++;}break;}}a(m).find("button[type=reset]").click(function(){d.reset(a(m));
...
a(t).find("option:selected").each(function(){p+=a(this).text()+", ";});if(p.length==0){v.html("&nbsp;");}else{v.text(p.substring(0,p.length-2));}}else{v.text("&nbsp;");
...
case"select":d.selectActivate(l,b,g);var k=a.extend(i,n.selectCollection);n.selectCollection=k;break;case"checkbox":if(typeof l!="string"){l.each(function(){d.checkboxActivate(a(this),g);
});}else{d.checkboxActivate(l,g);}break;case"radio":d.radioActivate(l,g);break;case"textarea":break;default:}return n;},elementDisable:function(k){if(typeof k=="string"){k=a(k);
}k.attr("disabled","disabled").closest(".jClever-element").addClass("disabled");},elementEnable:function(k){if(typeof k=="string"){k=a(k);}k.removeAttr("disabled").closest(".jClever-element").removeClass("disabled");
}};var f={};return this.each(function(){if(!a(this).hasClass("clevered")){a(this).addClass("clevered").addClass(j.selfClass);d.init(this);f={selectCollection:i,destroy:function(k){d.destroy(k);
},reset:function(k){d.reset(k);},selectSetPosition:function(k,l){if(a(k).length){d.selectSetPosition(a(k),l);}},selectAdd:function(k){d.selectAdd(k);},checkboxSetState:function(l,k){if(a(l).length){d.checkboxSetState(a(l),k);
}else{return false;}},radioSetState:function(k,l){if(a(k).length){d.radioSetState(a(k),l);}else{return false;}},elementAdd:function(k,l,m){return d.elementAdd(k,l,m);
},elementDisable:function(k){return d.elementDisable(k);},elementEnable:function(k){return d.elementEnable(k);}};i={};a.data(a(this).get(0),"publicApi",f);
return true;}});};a.fn.jCleverAPI=function(c){if(this.length>1){return false;}var f=a.data(a(this).get(0),"publicApi");var e=[];for(var d=1;d<arguments.length;
d++){e[d-1]=arguments[d];}if(typeof f[c]=="function"){if(arguments.length==1){e=new Array(a(this));}else{e[arguments.length-1]=f;}var b=f[c].apply(arguments.callee,e);
if(typeof b=="object"){a.data(a(this).get(0),"publicApi",b);}else{b=f;}return b;}else{return f[c];}};jQuery.jClever=true;a(document).mousedown(function(b){if(a(b.target).parents(".jClever-element-select-wrapper").length===0){a(".jClever-element-select-list-wrapper:visible").hide();
}});})(jQuery);