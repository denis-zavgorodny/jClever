...
}a(k).find("input[type=text], textarea").each(function(){var m=a(this);var l=a(this).data("placeholder");if(undefined!=l&&typeof l==="string"){if(m.val()==""){m.val(l).addClass("placeholdered");
...
}});}});if(j.applyTo.select){a(k).find("select").each(function(){c[a(this).attr("name")]={type:"select",value:a(this).attr("value")};d.selectActivate(this,b,g);
b--;g++;});}if(j.applyTo.checkbox){a(k).find("input[type=checkbox]").each(function(){c[a(this).attr("name")]={type:"checkbox",value:(a(this).is(":checked")?1:0)};
